...
const Dashboard = () => {
        ...
        const fetchData = async () => {
            try {
                const response = await axios.get(`${getBaseUrl()}/api/admin`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`, //esto se envia pero el middleware verifyUser no se aplica para este route
    ...
    // console.log(data)
    const percentageTrendingBooks = data?.totalBooks
        ? ((data.trendingBooks / data.totalBooks) * 100).toFixed(2)
        : '0.00';

    console.log(percentageTrendingBooks, "%");
    if (loading) return <Loading />
    //if (loading) return <div>Laoding ... Wait</div>
            ...
            <section className="grid md:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <div className="flex items-center p-8 bg-white shadow rounded-lg">
                    ...
                    <div>
                        ...
                        <span className="inline-block text-xl text-gray-500 font-semibold">{`(${percentageTrendingBooks} %)`}</span>
